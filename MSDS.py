...
# [UI 변경] 변환 버튼과 다운로드 버튼을 중앙에 배치
if 'converted_files' not in st.session_state:
    st.session_state['converted_files'] = []
    st.session_state['download_data'] = {}

uploaded_files = st.file_uploader("3. 원본 파일 업로드 (PDF)", type=["pdf"], accept_multiple_files=True)
st.write("") 
        ...
        with st.spinner(f"{option} 모드로 변환 중..."):
            ...
            try:
                ...
                if target_sheet:
                    ...
                    # [수정] row.columns 오류 해결 -> df_code.columns 사용
                    # 사용자 데이터가 K=C열, E=C열(영문내용)이라고 가정 (혹은 구조에 맞게)
