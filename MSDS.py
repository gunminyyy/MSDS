...
st.title("MSDS 양식 변환기 (테두리 제거 & 매핑 정밀 진단)")
st.markdown("---")

# --------------------------------------------------------------------------
# [스타일 정의] 굴림 8pt만 적용 (테두리 삭제)
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# [함수] 셀 스타일 적용 (굴림 8pt only, 테두리 제거)
# --------------------------------------------------------------------------
...
def write_ghs_data_final(ws, parsed_data, code_map):
    ...
    # 2. 섹션별 처리 (Offset 누적)
    current_offset = 0 # 지금까지 얼마나 행이 추가되었는가
    ...
    for section_name, codes, next_section_name in sections:
        ...
        if needed_rows > available_space:
            ...
            # 다음 헤더 위치(next_header_row) 바로 위에 삽입해야 함
            # insert_rows(idx)는 idx 위치에 새 행을 만들고 기존 idx 행을 아래로 내림
        ...
        if uploaded_files and master_data_file and template_file:
            with st.spinner("처리 중..."):
                ...
                try: 
                    ...
                    # [디버깅] 컬럼명 확인
                    # st.write("중앙 데이터 컬럼:", df_master.columns.tolist())
                    ...
                    # [디버깅] 매핑 맵 샘플 확인
                    # st.write("매핑 데이터 샘플 (상위 5개):", list(code_map.items())[:5])
                            
                except Exception as e:
                    st.error(f"중앙 데이터 로드 실패: {e}")
                ...
                for uploaded_file in uploaded_files:
                    if option == "CFF(K)":
                        try:
                            ...
                            # [디버깅] 추출된 코드 확인
                            # st.write(f"[{uploaded_file.name}] 추출된 코드:", parsed_data)
                ...
                if new_files:
                    st.success("완료! 테두리 제거 및 매핑 오류 수정됨.")
        else:
